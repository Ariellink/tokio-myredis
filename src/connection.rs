...
pub struct Connection {
    ...
    //所以我们要为Connection增加一个read buffer: socket->buffer->parse to freme -> remove the data from buffer
...
impl Connection {
    ...
    pub async fn read_frame(&mut self) -> Result<Option<Frame>> {
        loop {
            ...
            //tokio::tcpstream.read_buf(): Pulls some bytes from this source into the specified buffer, advancing the buffer's internal cursor. 将stream中的bytes推进缓冲区，推进缓冲区的内部cursor， return 读入的字节数。
    ...
    fn parse_frame(&mut self) -> Result<Option<Frame>> {
        //Cursor::new(&self.buffer[..]) 创建了一个新的 Cursor 对象，该对象可以对 self.buffer 中的数据进行读写操作，并且初始位置位于数据的开头。
        let mut buf = Cursor::new(&self.buffer[..]);
        //Frame::check 使用了 Buf 的字节迭代风格的 API。例如，为了解析一个帧，首先需要检查它的第一个字节，该字节用于说明帧的类型。这种首字节检查是通过 Buf::get_u8 函数完成的，该函数会获取游标所在位置的字节，然后将游标位置向右移动一个字节。
